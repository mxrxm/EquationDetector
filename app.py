"""
app.py — Streamlit frontend for the Equation Detector (standalone only).

Standalone-only build:
  - Inline detection removed; all results are class 0 (display equations)
  - Sidebar simplified: no inline tuning expander, no confidence threshold slider
  - Metrics: single standalone count card + density class card
  - Tab 1: Detection overlay (standalone boxes only)
  - Tab 2: Crops — every detected equation as a cropped image + ZIP download
  - Tab 3: Diagnostic (6-panel pipeline view)
  - Tab 4: Data table + confidence histogram + CSV download
"""
...
section[data-testid="stMain"] .block-container {
    padding-top: 3rem; padding-left: 3rem;
...
div[data-testid="stSidebar"] * { font-family: 'JetBrains Mono', monospace !important; }

.sb-logo { display:flex; align-items:center; gap:0.5rem; padding:0.2rem 0 0.1rem; }
.sb-logo-icon {
    font-size:1.1rem; color:#e8583a;
    font-family:'Syne',sans-serif !important; font-weight:800; line-height:1;
}
.sb-logo-name {
    font-family:'Syne',sans-serif !important; font-size:1rem;
    font-weight:700; color:#f0ede6; letter-spacing:-0.02em;
}
.sb-version {
    font-size:0.58rem; letter-spacing:0.2em; text-transform:uppercase;
    color:#383838; margin-top:0.1rem; margin-bottom:0.8rem;
}
.sb-section {
    font-size:0.58rem; letter-spacing:0.2em; text-transform:uppercase;
    color:#383838; padding-bottom:0.35rem; border-bottom:1px solid #1c1c20;
    margin-bottom:0.7rem; margin-top:1.1rem;
}
[data-testid="stRadio"] label { font-size:0.8rem !important; color:#ccc !important; }
...
.sb-footer { font-size:0.58rem; color:#2e2e2e; letter-spacing:0.06em; margin-top:0.5rem; }

.eq-hero { text-align:center; padding:2.5rem 0 0.5rem; margin-bottom:0.5rem; }
.eq-title {
    font-family:'Syne',sans-serif; font-size:3.4rem; font-weight:800;
    letter-spacing:-0.04em; color:#f0ede6; line-height:1; margin:0;
}
.eq-title-accent { color:#e8583a; }
.eq-subtitle {
    font-family:'JetBrains Mono',monospace; font-size:0.65rem; color:#444;
    letter-spacing:0.2em; text-transform:uppercase; margin-top:0.9rem;
}

.badge { display:inline-block; font-family:'JetBrains Mono',monospace; font-size:0.68rem;
         font-weight:700; letter-spacing:0.12em; text-transform:uppercase;
         padding:0.25rem 0.7rem; border-radius:2px; }
.badge-0 { background:#e8583a22; color:#e8583a; border:1px solid #e8583a55; }
.badge-1 { background:#a78bfa22; color:#a78bfa; border:1px solid #a78bfa55; }
.badge-2 { background:#4f9cf922; color:#4f9cf9; border:1px solid #4f9cf955; }

[data-testid="stTabs"] [data-baseweb="tab-list"] {
    background:transparent; border-bottom:1px solid #1e1e22; gap:0;
}
...
[data-testid="stTabs"] [aria-selected="true"] {
    color:#e8583a !important; border-bottom-color:#e8583a !important;
}

.stButton > button,
[data-testid="stDownloadButton"] > button {
    font-family:'JetBrains Mono',monospace; font-size:0.72rem;
    letter-spacing:0.05em; background:#1a1a1e; color:#d0cec8;
    border:1px solid #2a2a2e; border-radius:3px; padding:0.5rem 1.2rem;
    transition:all 0.15s;
}
.stButton > button:hover,
[data-testid="stDownloadButton"] > button:hover {
    background:#e8583a; border-color:#e8583a; color:#fff;
}

[data-testid="stAlert"] {
    background:#131316; border:1px solid #222; border-radius:4px; font-size:0.78rem;
}
[data-testid="stDataFrame"] { border:1px solid #222; border-radius:4px; font-size:0.78rem; }
...
.conf-badge {
    ...
    padding:0.12rem 0.45rem; border-radius:2px; background:#e8583a1a; color:#e8583a;
    border:1px solid #e8583a33; margin-left:0.4rem;
}
</style>
...
def _render_detections(gray, text_boxes, eq_results):
    ...
    for det in eq_results:
        ...
        ax.add_patch(patches.Rectangle(
            (x1, y1), x2-x1, y2-y1,
            linewidth=2, edgecolor="#e8583a", facecolor="#e8583a", alpha=0.12))
        ax.add_patch(patches.Rectangle(
            (x1, y1), x2-x1, y2-y1,
            linewidth=2, edgecolor="#e8583a", facecolor="none"))
        ax.text(x1+3, y1-4, f"{conf:.2f}",
                color="#e8583a", fontsize=6.5, fontweight="bold",
...
def _render_diagnostic_cached(gray_key, gray_flat, binary_flat, blob_data,
    ...
    blob_data  = [{"x1":t[0],"y1":t[1],"x2":t[2],"y2":t[3],
                  ...
                  for t in blob_data]
    eq_results = [{"box":t[0],"confidence":t[1],"class":t[2]} for t in eq_results]
    ...
    axes[0,2].axvline(font_size,        color="#f0ede6", lw=1.5, label=f"font={font_size}px")
    ...
    axes[0,2].legend(fontsize=7, facecolor=BG, edgecolor="#333", labelcolor="#aaa")
    ...
    for spine in axes[0,2].spines.values(): spine.set_edgecolor("#222")
    ...
    _style(axes[1,2], f"06 · detections · {len(eq_results)} standalone equations")
    axes[1,2].axis("off")
...
with st.sidebar:
    ...
    <div class="sb-version">Standalone Equation Detector</div>
    """, unsafe_allow_html=True)
...
<div class="eq-hero">
  ...
  <div class="eq-subtitle">Standalone Equation Localization &nbsp;·&nbsp; Connected Component Analysis</div>
</div>
""", unsafe_allow_html=True)
...
if uploaded is None:
    ...
    with _c2:
        st.markdown(
            ...
            "margin-top:1rem'>↑ Upload a document image to begin detection.</div>",
            unsafe_allow_html=True
        )
    st.stop()
...
if st.session_state.get("_cache_key") != _cache_key:
    ...
    # density_class comes back embedded in eq_results confidences; we derive
    # the label from the pipeline print output via a simple heuristic on blob count
    density_class = st.session_state.get("density_class", 1)
...
# METRICS
# ══════════════════════════════════════════════════════════════════════════════
# Derive density label from blob statistics (mirrors pipeline logic)
img_h = len(gray);  img_w = len(gray[0]) if img_h else 1
total_ink   = sum(b["area"] for b in blobs)
ink_density = total_ink / max(1, img_w * img_h)
body_blobs  = sum(1 for b in blobs
                  if font_size * 0.6 <= b["height"] <= font_size * 1.4)
body_ratio  = body_blobs / max(1, len(blobs))
est_lines   = img_h / max(1, font_size * 1.5)
bpl         = len(blobs) / max(1, est_lines)

def _vote(v, lo, hi): return -1 if v < lo else (1 if v >= hi else 0)
vs = _vote(ink_density, 0.025, 0.055) + _vote(body_ratio, 0.45, 0.65) + _vote(bpl, 12, 25)
density_class = 2 if vs == 3 else (0 if vs <= -1 else 1)
density_labels = {0: "SPARSE", 1: "MEDIUM", 2: "DENSE"}
density_label  = density_labels[density_class]

st.markdown(f"""
<div style="display:flex; gap:0.75rem; flex-wrap:wrap; margin-bottom:0.5rem;">

  <div style="flex:0 0 auto;width:180px;background:#131316;border:1px solid #222;
              border-radius:4px;border-top:2px solid #e8583a;padding:1.1rem 1.3rem;">
    <div style="font-family:'Syne',sans-serif;font-size:2.2rem;font-weight:800;
                color:#f0ede6;line-height:1;letter-spacing:-0.03em;">{len(eq_results)}</div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.6rem;color:#555;
  ...
  <div style="flex:0 0 auto;width:180px;background:#131316;border:1px solid #222;
              border-radius:4px;border-top:2px solid #4ade80;padding:1.1rem 1.3rem;">
    <div style="font-family:'Syne',sans-serif;font-size:2.2rem;font-weight:800;
                color:#f0ede6;line-height:1;letter-spacing:-0.03em;">{len(text_boxes)}</div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.6rem;color:#555;
                text-transform:uppercase;letter-spacing:0.12em;margin-top:0.45rem;">
      Text regions
    </div>
  ...
  <div style="flex:0 0 auto;width:160px;background:#131316;border:1px solid #222;
              border-radius:4px;border-top:2px solid #a78bfa;padding:1.1rem 1.3rem;">
    <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;
                color:#f0ede6;line-height:1;letter-spacing:-0.01em;margin-top:0.3rem;">
      <span class="badge badge-{density_class}">{density_label}</span>
    </div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.6rem;color:#555;
                text-transform:uppercase;letter-spacing:0.12em;margin-top:0.75rem;">
      Density class {density_class}
    </div>
  </div>

  <div style="flex:0 0 auto;width:160px;background:#131316;border:1px solid #222;
              border-radius:4px;border-top:2px solid #555;padding:1.1rem 1.3rem;">
    <div style="font-family:'Syne',sans-serif;font-size:2.2rem;font-weight:800;
                color:#f0ede6;line-height:1;letter-spacing:-0.03em;">{font_size}px</div>
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.6rem;color:#555;
                text-transform:uppercase;letter-spacing:0.12em;margin-top:0.45rem;">
      Estimated font size
    </div>
...
with tab1:
    det_fig  = _render_detections(gray, text_boxes, eq_results)
...
with tab2:
    ...
    if not eq_results:
        ...
        st.markdown(
            ...
            f"{len(crops)} equation{'s' if len(crops)!=1 else ''} extracted"
            f"</div>",
            unsafe_allow_html=True
        )
        ...
        with zipfile.ZipFile(zip_buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
            for c in crops:
                png = _pil_to_png_bytes(c["image"])
                fn  = f"eq_{c['index']:03d}_conf{c['confidence']:.2f}.png"
                crop_pngs.append((fn, png, c["confidence"]))
                zf.writestr(fn, png)
        zip_buf.seek(0)
        ...
        # Gallery — 3 columns
        for row_i in range((len(crop_pngs) + 2) // 3):
            cols = st.columns(3)
            for col_i, col in enumerate(cols):
                idx = row_i * 3 + col_i
                if idx >= len(crop_pngs): break
                fn, png, conf = crop_pngs[idx]
                with col:
                    st.markdown(
                        f"<div style='font-size:0.6rem;color:#555;letter-spacing:0.1em;"
                        f"text-transform:uppercase;margin-bottom:0.3rem'>"
                        f"EQ·{idx+1:03d}"
                        f"<span class='conf-badge'>conf {conf:.2f}</span></div>",
                        unsafe_allow_html=True
                    )
                    st.image(png, use_container_width=True)
                    st.download_button(
                        "⬇ Save", data=png, file_name=fn, mime="image/png",
                        key=f"crop_dl_{idx}", use_container_width=True,
                    )
...
with tab3:
    ...
    diag_data = _render_diagnostic_cached(
        ...
        tuple(
            (b["x1"],b["y1"],b["x2"],b["y2"],
             b["height"],b["width"],b["area"],b.get("fill_ratio",0))
            for b in blobs
        ),
        font_size,
        tuple(text_boxes),
        tuple((d["box"], d["confidence"], d["class"]) for d in eq_results),
        img_width, img_height,
    )
    ...
    rows = [
        {
            ...
            "width_px":  d["box"][2] - d["box"][0],
            "height_px": d["box"][3] - d["box"][1],
        }
        for d in eq_results
    ]
    ...
    if rows:
        try:
            ...
            st.markdown(
                ...
                "text-transform:uppercase;margin:1rem 0 0.5rem'>Confidence stats</div>",
                unsafe_allow_html=True
            )
            st.dataframe(df["confidence"].describe().round(3).to_frame(),
                         use_container_width=True)
        ...
        confs = [d["confidence"] for d in eq_results]
        ax_conf.hist(confs, bins=20, alpha=0.85, color="#e8583a",
                     label=f"Standalone ({len(confs)})", edgecolor="#0d0d0f")
        ax_conf.legend(fontsize=7, facecolor="#131316", edgecolor="#333", labelcolor="#aaa")
        ...
        for spine in ax_conf.spines.values(): spine.set_edgecolor("#222")
        plt.tight_layout()
        st.pyplot(fig_conf)
        plt.close(fig_conf)
    else:
